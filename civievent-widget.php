...
<?php
	...
	public static function regFix($event, $id, $classPrefix)
	{
		...
		if (
			...
			&& (
				($startDate = CRM_Utils_Array::value('registration_start_date', $event)) === null
				|| ($startDateObj = DateTimeImmutable::createFromFormat('Y-m-d H:i:s', $startDate)) instanceof DateTimeImmutable
				&& $startDateObj <= new DateTimeImmutable()
			)
			&& (
				($endDate = CRM_Utils_Array::value('registration_end_date', $event)) === null
				|| ($endDateObj = DateTimeImmutable::createFromFormat('Y-m-d H:i:s', $endDate)) instanceof DateTimeImmutable
